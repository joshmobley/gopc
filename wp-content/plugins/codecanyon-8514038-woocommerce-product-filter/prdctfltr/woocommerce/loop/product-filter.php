...
*/
	...
	$default_args = array(
		...
		'paged'					=> $paged,
		'orderby'				=> ( ( $pf_order = get_option( 'woocommerce_default_catalog_orderby' ) ) == 'menu_order' ? 'none' : $pf_order ),
		'meta_query'			=> array(
			array(
				'key'			=> '_visibility',
				'value'			=> array( 'catalog', 'visible' ),
	...
	if ( isset( $prdctfltr_global['unique_id'] ) ) {
		...
		$prdctfltr_global['filter_js'][$prdctfltr_id]['args'] = $default_args;
		$prdctfltr_global['filter_js'][$prdctfltr_id]['atts'] = isset( $prdctfltr_global['ajax_js'] ) ? $prdctfltr_global['ajax_js'] : array() ;
	}
	else {
...
?>
	...
	<?php
						...
						switch( $k ) {
							...
							case 'min_price' :
								if ( isset( $pf_activated['min_price'] ) && $pf_activated['min_price'] !== '' ) {
									...
									echo ' / <span>' . wc_price( $min_price ) . ' - ' . wc_price( $max_price ) . '</span> <a href="#" class="prdctfltr_title_remove" data-key="byprice"><i class="prdctfltr-delete"></i></a>';
								}
							break;
							case 'max_price' :
							break;
							case 'rng_min_price' :
								if ( isset( $pf_activated['rng_min_price'] ) && $pf_activated['rng_min_price'] !== '' ) {
									...
									echo ' / <span>' . __('Price range', 'prdctfltr') . ' ' . wc_price( $min_price ) . ' &rarr; ' . wc_price( $max_price ) . '</span> <a href="#" class="prdctfltr_title_remove" data-key="byprice"><i class="prdctfltr-delete"></i></a>';
								}
							break;
							case 'rng_max_price' :
							break;
							default :
								if ( substr( $k, 0, 4 ) == 'rng_' ) {
									...
									$curr_selected['min'] = $v;
									$curr_selected['max'] = ( isset( $pf_activated['rng_max_' . $true_val] ) ? $pf_activated['rng_max_' . $true_val] : '' );
									...
									foreach( $curr_selected as $selected ) {
										if ( term_exists( $selected, $k ) ) {
											$curr_term = get_term_by( 'slug', $selected, $k );
											echo ( $i !== 0 ? ', ' : '' ) . $curr_term->name;
											$i++;
										}
									}
									echo '</span> <a href="#" class="prdctfltr_title_remove" data-key="' . ( $k == 'characteristics' ? 'char' : $k ) . '"><i class="prdctfltr-delete"></i></a>';
		...
		if ( $total > 0 ) {
			if ( $curr_options['wc_settings_prdctfltr_adoptive'] == 'yes' || ( defined('DOING_AJAX') && DOING_AJAX ) ) {
				...
				$request = isset( $pf_query->request ) ? $pf_query->request : '';
					...
					?>
						<div class="prdctfltr_filter prdctfltr_per_page<?php echo $curr_term_customization; ?>" data-filter="pf_per_page">
							...
							<?php
								...
								if ( $curr_options['wc_settings_prdctfltr_disable_showresults'] == 'no' && ( $curr_styles[5] == 'prdctfltr_disable_bar' || isset( $prdctfltr_global['widget_style'] ) ) && isset($pf_activated['products_per_page'] ) ) {
					...
					?>
						<div class="prdctfltr_filter prdctfltr_instock<?php echo $curr_term_customization; ?>" data-filter="pf_instock">
							...
							<?php
								...
								if ( $curr_options['wc_settings_prdctfltr_disable_showresults'] == 'no' && ( $curr_styles[5] == 'prdctfltr_disable_bar' || isset( $prdctfltr_global['widget_style'] ) ) && isset($pf_activated['instock_products'] ) ) {
					...
					?>
						<div class="prdctfltr_filter prdctfltr_orderby<?php echo $curr_term_customization; ?>" data-filter="orderby">
							...
							<?php
								...
								if ( $curr_options['wc_settings_prdctfltr_disable_showresults'] == 'no' && ( $curr_styles[5] == 'prdctfltr_disable_bar' || isset( $prdctfltr_global['widget_style'] ) ) && isset( $pf_activated['orderby'] ) && isset( $catalog_orderby[$pf_activated['orderby']] ) ) {
					...
					?>
						<div class="prdctfltr_filter prdctfltr_byprice<?php echo $curr_term_customization; ?>"  data-filter="pf_byprice">
							...
							<?php
								...
								if ( $curr_options['wc_settings_prdctfltr_disable_showresults'] == 'no' && ( $curr_styles[5] == 'prdctfltr_disable_bar' || isset( $prdctfltr_global['widget_style'] ) ) && isset($pf_activated['min_price']) && $pf_activated['min_price'] !== '' ) {
									$min_price = wc_price( $pf_activated['min_price'] );
									if ( isset( $pf_activated['max_price'] ) && $pf_activated['max_price'] !== '' ) {
										$curr_max_price = $pf_activated['max_price'];
										$max_price = wc_price( $pf_activated['max_price'] );
									}
									else {
										$max_price = ' +';
									}
							...
							<?php
								...
								if ( $curr_options['wc_settings_prdctfltr_disable_showresults'] == 'no' && ( $curr_styles[5] == 'prdctfltr_disable_bar' || isset( $prdctfltr_global['widget_style'] ) ) ) {
									if ( isset($pf_activated['rng_min_' . $attr]) && isset($pf_activated['rng_max_' . $attr]) ) {
										...
										if ( $attr == 'price' ) {
											echo wc_price($pf_activated['rng_min_' . $attr]) . ' - ' . wc_price($pf_activated['rng_max_' . $attr]);
										}
										else {
							...
							<?php
								...
								if ( !in_array($curr_options['wc_settings_prdctfltr_range_filters']['pfr_taxonomy'][$p], array('price') ) ) {
									...
									foreach ( $curr_attributes as $attribute ) {
										...
										if ( isset($pf_activated['rng_min_' . $attr]) && isset($pf_activated['rng_max_' . $attr]) ) {
											if ( $pf_activated['rng_min_' . $attr] == $attribute->slug ) {
												$pf_curr_min = $c;
											}
											if ( $pf_activated['rng_max_' . $attr] == $attribute->slug ) {
												$pf_curr_max = $c;
											}
										}
									...
									if ( ( isset($pf_activated['rng_min_' . $attr]) && isset($pf_activated['rng_max_' . $attr]) ) !== false ) {
										$pf_curr_min = ( isset($pf_activated['rng_min_' . $attr]) ? $pf_activated['rng_min_' . $attr] : $pf_activated['min_' . $attr] );
										$pf_curr_max = ( isset($pf_activated['rng_max_' . $attr]) ? $pf_activated['rng_max_' . $attr] : $pf_activated['max_' . $attr] );
									}
								}
									...
									$pf_add_settings .= '
										onFinish: function (data) {
											if ( data.min == data.from && data.max == data.to ) {
												...
												$(\'#' . $curr_rng_id . '\').closest(\'.prdctfltr_filter\').find(\'input[name^="rng_min_"]:first\').val( ( data.from_value == null ? parseInt(data.from)*priceRatio : parseInt($(data.from_value).text())*priceRatio ) );
												$(\'#' . $curr_rng_id . '\').closest(\'.prdctfltr_filter\').find(\'input[name^="rng_max_"]:first\').val( ( data.to_value == null ? parseInt(data.to)*priceRatio : parseInt($(data.to_value).text())*priceRatio ) ).trigger(\'change\');
											}
										}';
								}
	...
	(function($){
	"use strict";
		var priceRatio = 100/<?php echo WC_Prdctfltr::price_to_float(strip_tags(wc_price(100))); ?>;
	...
	<?php
					...
					?>
						<div class="prdctfltr_filter prdctfltr_search" data-filter="pf_search">
							...
							<?php
								...
								if ( $curr_options['wc_settings_prdctfltr_disable_showresults'] == 'no' && ( $curr_styles[5] == 'prdctfltr_disable_bar' || isset( $prdctfltr_global['widget_style'] ) ) && isset($pf_activated['s'] ) ) {
									echo '<a href="#" data-key="s"><i class="prdctfltr-delete"></i></a> <span>'.$catalog_instock[$pf_activated['s']] . '</span> / ';
								}
							...
							<?php
								$curr_insert = '<input class="pf_search" name="s" type="text"' . ( isset($pf_activated['s'] ) ? ' value="'.$pf_activated['s'].'"' : '' ) . ' placeholder="' . esc_attr( __( 'Product keywords', 'prdctfltr' ) ) . '">';
				...
				<?php
					...
					default :
						...
						?>
							<div class="prdctfltr_filter prdctfltr_attributes prdctfltr_<?php echo $curr_el; ?> <?php echo $curr_term_style; ?><?php echo $curr_term_multi; ?><?php echo $curr_term_customization; ?><?php echo $curr_term_adoptive; ?><?php echo $curr_term_relation; ?><?php echo $curr_term_expand; ?>" data-filter="<?php echo $curr_fo['filter']; ?>" data-limit="<?php echo $curr_limit !== 0 ? $curr_limit-1 : '0';?>">
								<input name="<?php echo $curr_fo['filter']; ?>" type="hidden"<?php echo ( !empty( $curr_cat_selected ) ? isset( $prdctfltr_global['taxonomies_data'][$curr_fo['filter'].'_string'] ) ? ' value="' . esc_attr( $prdctfltr_global['taxonomies_data'][$curr_fo['filter'].'_string'] ) . '"' : '' : '' ); ?> />
								<?php
									if ( isset($prdctfltr_global['widget_style']) ) {
										$pf_before_title = $before_title . '<span class="prdctfltr_widget_title">';
										$pf_after_title = '</span>' . $after_title;
								...
								<?php
									...
									foreach ( $catalog_categories as $term ) {
										...
										if ( $curr_fo['settings']['customization'] == '' ) {
											...
											$term_count = ( $curr_options['wc_settings_prdctfltr_show_counts'] == 'no' || $term->count == '0' ? '' : ' <span class="prdctfltr_count">' . ( isset($output_terms[$curr_fo['filter']]) && isset($output_terms[$curr_fo['filter']][$term->slug]) && $output_terms[$curr_fo['filter']][$term->slug] != $term->count ? WC_Prdctfltr::get_term_count( $output_terms[$curr_fo['filter']][$term->slug], $term->count ) : $term->count ) . '</span>' );
											...
											$term_count = ( $curr_options['wc_settings_prdctfltr_show_counts'] == 'no' || $term->count == '0' ? false : ( isset($output_terms[$curr_fo['filter']]) && isset($output_terms[$curr_fo['filter']][$term->slug]) && $output_terms[$curr_fo['filter']][$term->slug] != $term->count ? WC_Prdctfltr::get_term_count( $output_terms[$curr_fo['filter']][$term->slug], $term->count ) : $term->count ) );
										...
										if ( isset( $curr_fo['settings']['hierarchy'] ) && $curr_fo['settings']['hierarchy'] == 'yes' && !empty( $pf_children ) ) {
											...
											foreach( $pf_children as $sub ) {
												...
												$curr_insert = $sub->name . ( $curr_options['wc_settings_prdctfltr_show_counts'] == 'no' || $sub->count == '0' ? '' : ' <span class="prdctfltr_count">' . ( isset($output_terms[$curr_fo['filter']]) && isset($output_terms[$curr_fo['filter']][$sub->slug]) && $output_terms[$curr_fo['filter']][$sub->slug] != $sub->count ? WC_Prdctfltr::get_term_count( $output_terms[$curr_fo['filter']][$sub->slug], $sub->count ) : $sub->count ) . '</span>' );
												...
												if ( !empty($sub->children) ) {
													...
													foreach( $sub->children as $subsub ) {
														...
														$curr_insert = $subsub->name . ( $curr_options['wc_settings_prdctfltr_show_counts'] == 'no' || $subsub->count == '0' ? '' : ' <span class="prdctfltr_count">' . ( isset($output_terms[$curr_fo['filter']]) && isset($output_terms[$curr_fo['filter']][$subsub->slug]) && $output_terms[$curr_fo['filter']][$subsub->slug] != $subsub->count ? WC_Prdctfltr::get_term_count( $output_terms[$curr_fo['filter']][$subsub->slug], $subsub->count ) : $subsub->count ) . '</span>' );
														...
														if ( !empty($subsub->children) ) {
															...
															foreach( $subsub->children as $subsubsub ) {
																...
																$curr_insert = $subsubsub->name . ( $curr_options['wc_settings_prdctfltr_show_counts'] == 'no' || $subsubsub->count == '0' ? '' : ' <span class="prdctfltr_count">' . ( isset($output_terms[$curr_fo['filter']]) && isset($output_terms[$curr_fo['filter']][$subsubsub->slug]) && $output_terms[$curr_fo['filter']][$subsubsub->slug] != $subsubsub->count ? WC_Prdctfltr::get_term_count( $output_terms[$curr_fo['filter']][$subsubsub->slug], $subsubsub->count ) : $subsubsub->count ) . '</span>' );
		...
		<?php
			...
			if ( !isset( $prdctfltr_global['sc_query'] ) ) {
				if ( isset( $pf_activated['product_cat'] ) && $curr_cat_query !== 'yes' ) {
					$pf_queried_term = $prdctfltr_global['taxonomies_data']['product_cat_string'];

					echo '<input type="hidden" name="product_cat" value="' . $pf_queried_term . '" />';
					$prdctfltr_global['filter_js'][$prdctfltr_id]['adds']['product_cat'] = $pf_queried_term;
				}
				else if ( !isset( $pf_activated['product_cat'] ) && $curr_cat_query !== 'yes' && ( $pf_queried_term = get_query_var( 'product_cat' ) ) !== '' ) {
					echo '<input type="hidden" name="product_cat" value="' . $pf_queried_term . '" />';
					$prdctfltr_global['filter_js'][$prdctfltr_id]['adds']['product_cat'] = $pf_queried_term;
				}
			}
